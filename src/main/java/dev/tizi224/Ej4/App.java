package dev.tizi224.Ej4;

import dev.tizi224.Ej1.excepciones.CupoExcedidoException;
import dev.tizi224.Ej4.hilos.EnvioTicketsThread;
...
import dev.tizi224.Ej4.modelo.Sala;
import dev.tizi224.Ej4.modelo.actividades.Actividad;
import dev.tizi224.Ej4.modelo.actividades.Charla;
import dev.tizi224.Ej4.modelo.actividades.Curso;
import dev.tizi224.Ej4.modelo.actividades.Taller;
...
public class App {
    ...
    public static void main(String[] args) {
        ...
        EventoUniversitario evento2 = new EventoUniversitario("E2", "Workshop de Programación", 5000.0, false);

        Sala sala = new Sala("Sala Principal");
        Sala sala2 = new Sala("Sala Secundaria");
        evento.agregarSala(sala);
        evento2.agregarSala(sala2);
        ...
        for(EventoUniversitario event : eventos){
            for(Taller t : evento.filtrarActividadesPorTipo(Taller.class)){
                for (Inscripcion inscripcion : t.getInscripciones()) {
                    System.out.println(t.generarCertificado(inscripcion.getEstudiante()));
                }
            }
            for (Curso c :  evento.filtrarActividadesPorTipo(Curso.class)){
        ...
        System.out.println("--- Filtramos las listas de actividades ---");
        System.out.println("--- Talleres ---");
        System.out.println("Cantidad de talleres: " + evento.filtrarActividadesPorTipo(Taller.class).size());
        System.out.println(evento.filtrarActividadesPorTipo(Taller.class).toString());
        System.out.println("--- Cursos ---");
        System.out.println("Cantidad de cursos: " + evento.filtrarActividadesPorTipo(Curso.class).size());
        System.out.println(evento.filtrarActividadesPorTipo(Curso.class).toString());
        System.out.println("--- Charlas ---");
        System.out.println("Cantidad de charlas: " + evento.filtrarActividadesPorTipo(Charla.class).size());
        System.out.println(evento.filtrarActividadesPorTipo(Charla.class).toString());

        System.out.println("--- Calculando el costo de los materiales de cada actividad ---");
        for (Actividad actividad : evento.getActividades()) {
            System.out.println("Costo de materiales para " + (actividad.getClass().getSimpleName() + ": " + actividad.calcularCostoMateriales()));
        }

        insc1.setEstado("Confirmada");
        if(insc1.getEstado().equals("Confirmada")){
            Inscripcion.TicketDeAcceso ticket = insc1.new TicketDeAcceso("1", LocalDate.now());
            insc1.setTicket(ticket);
        }

        EnvioTicketsThread hiloEnvios = new EnvioTicketsThread(evento);
        hiloEnvios.start();

        // A partir de ahora se están corriendo 2 hilos paralelos. Vamos a evidenciarlo con un sout
        ...
        for (int i = 0; i < 5; i++) {
            System.out.println("Esta es otra tarea paralela del hilo principal");
            try{
                Thread.sleep(300);
